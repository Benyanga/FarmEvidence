...
const economicRecordEntrySchema = new Schema({
  ...
  total_plot_rwf: {
    ...
    // For Labour: decimal_days × wage_rate (from TrialSeason)
  },
  total_ha_rwf: {
    type: Number,
    default: null,
...
economicRecordEntrySchema.pre('save', async function(next) {
  ...
  const season = await mongoose.model('TrialSeason')
    ...
    .select('extrapolation_factor wage_rate_rwf_day');

  // Compute total_plot_rwf
  if (this.sub_category === 'Labour' && this.decimal_days != null) {
    this.total_plot_rwf = this.decimal_days * season.wage_rate_rwf_day;
  } else {
    this.total_plot_rwf = this.quantity * this.unit_cost_rwf;
  }
...
module.exports = mongoose.model('EconomicRecordEntry', economicRecordEntrySchema);